...
ori_img1 = cv2.imread('labels5.png')
# 中值滤波，去噪
# ori_img1 = ori_img1[6500:8660,6000:8000]
ori_img1 = 255 - ori_img1
...
fill_img = np.zeros((ori_img.shape), np.uint8)
# 遍历联通域
for i in range(1, num_labels):
    img = np.zeros_like(labels)
    index = np.where(labels==i)
    img[index] = 255
    img = np.array(img, dtype=np.uint8) # cv2.FILLED 填充
    ...
    cv2.drawContours(fill_img, [regularization_contour], -1, (255, 255, 255), cv2.FILLED)
...
cv2.imwrite('labels5reg.jpg', fill_img)

cv2.namedWindow("vis",0)
cv2.resizeWindow("vis", 1000, 900)
cv2.imshow('vis', ori_img1)
